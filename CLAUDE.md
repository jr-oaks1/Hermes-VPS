...
> ## 🟢 S21 — Stale PostgreSQL roles table reconciled live; last owned pending closed (2026-09-10)
> Housekeeping. Closed P2 from S19b: rewrote the `### PostgreSQL roles` subsection of `docs/VPS_CONNECTIVITY_REFERENCE.md` §5 (was ~9 mixed-host rows, stale since the crypto_signals→Contabo move + hermes_v2 decommission + least-privilege DB split) from a **live read-only inventory** on both hosts — new databases mini-table, full Hetzner `:5432` table (18 login roles + `vps_orchestrator` NOLOGIN, with Owns / Used-by / provenance), compact Contabo `:5434` list, and the 8-rule cross-host `pg_hba` grant matrix. **No infra changes** (SSH `psql` read-only only). One observation logged, not actioned: `crypto_platform` is a login role with `CONNECT`+`USAGE` but zero table grants and no confirmable consumer — candidate for review/`DROP` next session. Host `running`, 0 failed units, 0 open findings, replication `streaming/async/0`, disk 54%. **Nothing JR-Hermes-VPS-owned is open** — all remaining pendings (X1/X3/P3/Y1–Y4) are GM / Ingestor / Clevious. See `docs/sessions/S21-HANDOFF.md`.
---
