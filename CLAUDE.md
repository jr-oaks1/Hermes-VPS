...
> ## 🟢 S22 — B8/B3 closed, cross-project sweep, firewall re-audit, HERMES_PLATFORM_STANDARD.md R3 rule, Cloudflare access for Claude Code (2026-09-11)
> **B8**: live-verified `crypto_platform` was a genuine orphan (0 connections, 0 grants in `hermes_v2`, no `crypto*` DB, no `/opt/crypto_platform`, no systemd refs — the only surviving mentions are the decommissioned `crypto_data` project's stale bootstrap files) — `pg_dumpall --roles-only` snapshot, `BEGIN/ROLLBACK` smoke test, then `deploy/sql/S22_drop_crypto_platform_role.sql` applied for real (its own `pg_shdepend` hard-gate returned 0 rows first). Dropped, verified gone; `VPS_CONNECTIVITY_REFERENCE.md` 18→17 login roles. **B3**: `sshd`'s `ListenAddress` is inert under this host's socket-activation, so the real fix is an `ssh.socket` drop-in — staged, `systemd-analyze verify`d, applied with a 10-min armed auto-revert, both `:22` (Tailscale) and `:52222` (public fallback) confirmed reachable from fresh connections, **then** disarmed. `deploy/ssh.socket.d/20-jr-bind-scope.conf` now tracked. **Cross-project sweep**: **Y1 + Y2 confirmed already closed** by Clevious VPS S53/S57 (parity-check wiring live+quiet; standby GUC headroom holding at 16/1024); A1/A2/X1/X3/P3/Y3(B7)/Y4 all re-confirmed still genuinely waiting on their owners. **Deep forensic audit** (no CRIT/HIGH) + **full doc audit** of `VPS_CONNECTIVITY_REFERENCE.md` (§5/pg_hba accurate; **§7 "Active Services by Node" flagged stale/self-contradicting** — lists decommissioned `hermes_v2.service` as active and a `crypto-health-monitor.service` that no longer exists on the host at all — not fixed, flagged only). **Backup setup confirmed live**: full 3-2-1 holds for 5 DBs; `hermes_ingestor_log` still zero coverage (A1); `hermes_vps_log`+`vps_orchestrator_findings` missing only the Drive leg (new: **A3**). **SSH-port-alignment + hardening plan drafted** (`docs/SSH_PORT_ALIGNMENT_AND_HARDENING_PLAN.md`) — Hetzner's split-port shape is the safer of the two hosts; Contabo recommended to match, not the reverse; **live firewall re-audit** (UFW fully verified + Hetzner Cloud `firewall-1` confirmed via user console screenshot — 5 rules, matches doc + empirical probe exactly; new finding **H9**, Cloudflare-CIDR restriction on 80/443 is UFW-only, no cloud-firewall redundancy). **`HERMES_PLATFORM_STANDARD.md` §3 R3 updated** (workspace root + `/opt/`, byte-identical) — new binding rule: GM must own a periodic backup-enrollment reconciliation check so a missed opt-in like A1/A3 can't recur silently (detect-and-report only). **Cloudflare API access set up for Claude Code, workspace-wide**: `cloudflare@cloudflare` plugin installed at `Scope: user` (all projects, post-`/reload-plugins`); a scoped, IP-restricted-to-Hetzner API token created, live-verified working (`zone_settings:edit`/`dns_records:edit`/`zone:edit`/`cache_purge:edit`/`app:edit` on `artek-studio.com`), stored at workspace root `cloudflare-credentials.env` (deliberately outside the encrypted `_credentials/` vault, documented as a reasoned exception in its README + `AUDIT_LOG.md`) — **all Cloudflare calls must relay through Hetzner**, the token doesn't work from anywhere else. Two cross-project notices sent (A1/A3 backup enrollment → Ingestor+GM; SSH hardening proposal → Clevious VPS). **Session-numbering conflict caught at close-out** — user asked for this handoff as "S21"; `S21-HANDOFF.md` already existed as the real prior session, so the conflict was surfaced per `SESSION_NUMBERING_STANDARD.md` rather than silently resolved either way; confirmed S22 is correct. **No JR-Hermes-VPS-owned pending remains** (C1/C2/C3 are proposals/optional follow-ups, not open defects). Host `running`, 0 failed units, 0 open findings, replication `streaming/async/0` (0 B lag), disk 47%, `deploy_guardrail.sh` 9/9. See `docs/sessions/S22-HANDOFF.md`.
---
