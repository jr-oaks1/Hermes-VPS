...
**Date:** 2026-09-07. *(Harness clock reported 2026-09-06 this session — dated 2026-09-07 for
chronological consistency with the committed S19; flag noted.)*
...
## VERIFICATION (live, 2026-09-07)

<!-- filled in after live apply -->
- [ ] `pg_dumpall --roles-only` backup + `pg_hba.conf.bak-s20` taken
- [ ] SQL smoke-tested in staged worktree (`BEGIN; … ROLLBACK;`) — asserts pass
- [ ] SQL applied for real — all step-5/6/7 asserts as expected
- [ ] `pg_hba` line added + `pg_reload_conf()`
- [ ] positive: `psql "host=100.97.62.7 dbname=parity user=parity_reader"` → 5 GUC rows
- [ ] negative: same role → connect `hermes_v2` denied; `pg_authid` read denied
- [ ] `deploy_guardrail.sh` exit 0
- [ ] host `is-system-running`=running, 0 failed, replication `streaming/async/0`, disk ~56%
- [ ] `/opt/HERMES_PLATFORM_STANDARD.md` md5 == workspace
- [ ] T-LOG.1 note row in `findings_log` + `@JRHermesVPSBot`
- [ ] credential written to `_credentials/jr_hermes_vps/parity_reader.md` + `/root/.hermes_vps_credentials/CREDENTIALS.md`
